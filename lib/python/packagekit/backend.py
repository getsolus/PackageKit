...
class PackageKitBaseBackend:
    ...
    def details(self, package_id, summary, package_license, group, desc, url, bytes, dl_bytes):
        '''
        Send 'details' signal
        @param package_id: The package ID name, e.g. openoffice-clipart;2.6.22;ppc64;fedora
        @param summary: The package summary
        ...
        @param dl_bytes: The download size of the package, in bytes
        '''
        sys.stdout.write(_to_utf8("details\t%s\t%s\t%s\t%s\t%s\t%s\t%ld\t%ld\n" % (package_id, summary, package_license, group, desc, url, bytes, dl_bytes)))
        sys.stdout.flush()
